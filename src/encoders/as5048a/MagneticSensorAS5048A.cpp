...
float MagneticSensorAS5048A::getAngle() {
    ...
    if(abs(d_angle) > (0.8*AS5048A_CPR) ) full_rotation_offset += d_angle > 0 ? -_2PI : _2PI; 
    // save the current angle value for the next steps
    // in order to know if overflow happened
    angle_data_prev = angle_data;
    angle_curr = full_rotation_offset + ( angle_data / (float)AS5048A_CPR) * _2PI;
...
*/
float MagneticSensorAS5048A::getVelocity() {
